...
func (n *dirNode) Statx(ctx context.Context, f fs.FileHandle, flags uint32, mask uint32, out *fuse.StatxOut) syscall.Errno {
	...
	fillStatx(&out.Statx, &attr.Attr, mask)
	n.backend.log.Debug("pathstub statx", "flags", flags, "mask", mask)
	return 0
}

func (n *fileNode) Statx(ctx context.Context, f fs.FileHandle, flags uint32, mask uint32, out *fuse.StatxOut) syscall.Errno {
	var attr fuse.AttrOut
	if errno := n.Getattr(ctx, f, &attr); errno != 0 {
		return errno
	}
	fillStatx(&out.Statx, &attr.Attr, mask)
	n.backend.log.Debug("pathstub statx", "name", n.name, "flags", flags, "mask", mask)
	return 0
}

func fillStatx(out *fuse.Statx, attr *fuse.Attr, mask uint32) {
	out.Mask = mask
