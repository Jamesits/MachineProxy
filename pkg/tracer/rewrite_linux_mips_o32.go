...
// MIPS o32 syscall numbers (4000 base).
const (
	sysExecve   = 4011
	sysExecveat = 4356
)
...
// SyscallRegs holds the register state for a syscall on MIPS o32.
// On execve(pathname, argv, envp):
//   - v0 = syscall number (with offset 4000)
//   - a0 = pathname pointer
//   - a1 = argv pointer
//   - a2 = envp pointer
//
...
//   - a2 = argv pointer
//   - a3 = envp pointer
//   - flags is passed on the stack (o32 only has 4 arg registers)
...
func (r *SyscallRegs) PathAddr(isExecveat bool) uintptr {
	if isExecveat {
		return uintptr(uint32(r.regs.Regs[mipsA1]))
	}
	return uintptr(uint32(r.regs.Regs[mipsA0]))
}

func (r *SyscallRegs) ArgvAddr(isExecveat bool) uintptr {
	if isExecveat {
		return uintptr(uint32(r.regs.Regs[mipsA2]))
	}
	return uintptr(uint32(r.regs.Regs[mipsA1]))
}
...
func (r *SyscallRegs) EnvpAddr(isExecveat bool) uintptr {
	if isExecveat {
		return uintptr(uint32(r.regs.Regs[mipsA3]))
	}
	return uintptr(uint32(r.regs.Regs[mipsA2]))
}
...
func (r *SyscallRegs) SetPathAddr(isExecveat bool, addr uintptr) {
	if isExecveat {
		r.regs.Regs[mipsA1] = uint64(uint32(addr))
	} else {
		r.regs.Regs[mipsA0] = uint64(uint32(addr))
	}
}

func (r *SyscallRegs) SetArgvAddr(isExecveat bool, addr uintptr) {
	if isExecveat {
		r.regs.Regs[mipsA2] = uint64(uint32(addr))
	} else {
		r.regs.Regs[mipsA1] = uint64(uint32(addr))
	}
}
...
func (r *SyscallRegs) SetEnvpAddr(isExecveat bool, addr uintptr) {
	if isExecveat {
		r.regs.Regs[mipsA3] = uint64(uint32(addr))
	} else {
		r.regs.Regs[mipsA2] = uint64(uint32(addr))
	}
}
...
func (r *SyscallRegs) StackPointer() uintptr {
	return uintptr(uint32(r.regs.Regs[mipsSP]))
}
...
func (r *SyscallRegs) Ret() (uintptr, bool) {
	return uintptr(uint32(r.regs.Regs[mipsV0])), r.regs.Regs[mipsA3] == 0
}

func (r *SyscallRegs) SetRetSuccess(v uintptr) {
	r.regs.Regs[mipsV0] = uint64(uint32(v))
	r.regs.Regs[mipsA3] = 0
}

func (r *SyscallRegs) SetRetError(errno uintptr) {
	r.regs.Regs[mipsV0] = uint64(uint32(errno))
